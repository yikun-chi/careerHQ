"""
occupation_initialize.py
=========================

This module initializes all the static data structures needed for the O*NET occupation model:
...
def load_scale_definitions(file_path: Optional[Path] = None) -> Dict[str, ScaleDefinition]:
    """
    Load scale definitions from occupation_scale_reference.txt or occupation_scale_def.csv.
    ...
    if file_path is None:
        ...
        if (data_dir / "occupation_scale_def.csv").exists():
            file_path = data_dir / "occupation_scale_def.csv"
        else:
            file_path = data_dir / "occupation_scale_reference.txt"
