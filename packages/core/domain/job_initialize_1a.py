...
Data sources:
- job_element_scale_mapping_1A.csv: Maps elements to scales
...
def load_im_scale_schema(file_path: Optional[Path] = None) -> tuple[ScaleDefinition, OrdinalSemantics]:
    ...
    if file_path is None:
        file_path = get_data_dir() / "job_scale_im_schema.json"
...
def load_element_scale_mapping(file_path: Optional[Path] = None) -> Dict[str, list[str]]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_element_scale_mapping_1A.csv"
