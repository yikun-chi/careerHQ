...
Data sources:
- job_element_scale_mapping_1D.csv: Maps elements to scales
- job_scale_wi_schema.json: WI scale definition
- job_scale_dr_schema.json: DR scale definition
"""
...
def load_wi_scale_schema(file_path: Optional[Path] = None) -> tuple[ScaleDefinition, IntervalSemantics]:
    ...
    if file_path is None:
        file_path = get_data_dir() / "job_scale_wi_schema.json"
...
def load_dr_scale_schema(file_path: Optional[Path] = None) -> tuple[ScaleDefinition, OrdinalSemantics]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_scale_dr_schema.json"
...
def load_element_scale_mapping_1d(file_path: Optional[Path] = None) -> Dict[str, list[str]]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_element_scale_mapping_1D.csv"
