...
Data sources:
- job_element_scale_mapping_2D3A.csv: Maps elements to scale prefixes
- job_scale_rl_schema.json: RL category definitions (12 categories)
- job_scale_rw_schema.json: RW category definitions (11 categories)
- job_scale_pt_schema.json: PT category definitions (9 categories)
- job_scale_oj_schema.json: OJ category definitions (9 categories)
- job_scale_im_schema.json: IM scale (reused from 1.A)
"""
...
from .job_initialize import get_elements, get_data_dir
...
def load_rl_scale_schemas(file_path: Optional[Path] = None) -> Dict[int, Tuple[ScaleDefinition, IntervalSemantics]]:
    ...
    if file_path is None:
        file_path = get_data_dir() / "job_scale_rl_schema.json"
...
def load_rw_scale_schemas(file_path: Optional[Path] = None) -> Dict[int, Tuple[ScaleDefinition, IntervalSemantics]]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_scale_rw_schema.json"
...
def load_pt_scale_schemas(file_path: Optional[Path] = None) -> Dict[int, Tuple[ScaleDefinition, IntervalSemantics]]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_scale_pt_schema.json"
...
def load_oj_scale_schemas(file_path: Optional[Path] = None) -> Dict[int, Tuple[ScaleDefinition, IntervalSemantics]]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_scale_oj_schema.json"
...
def load_element_scale_mapping_2d3a(file_path: Optional[Path] = None) -> Dict[str, List[str]]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_element_scale_mapping_2D3A.csv"
