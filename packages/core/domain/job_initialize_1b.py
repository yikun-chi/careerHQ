...
Data sources:
- job_element_scale_mapping_1B.csv: Maps elements to scales
- job_scale_oi_schema.json: OI scale definition
- job_scale_ex_schema.json: EX scale definition
"""
...
def load_oi_scale_schema(file_path: Optional[Path] = None) -> tuple[ScaleDefinition, IntervalSemantics]:
    ...
    if file_path is None:
        file_path = get_data_dir() / "job_scale_oi_schema.json"
...
def load_ex_scale_schema(file_path: Optional[Path] = None) -> tuple[ScaleDefinition, IntervalSemantics]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_scale_ex_schema.json"
...
def load_element_scale_mapping_1b(file_path: Optional[Path] = None) -> Dict[str, list[str]]:
    ...
    """
    if file_path is None:
        file_path = get_data_dir() / "job_element_scale_mapping_1B.csv"
